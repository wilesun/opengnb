...
static void init_ctl_block(gnb_core_t *gnb_core, gnb_conf_t *conf) {
    ...
    if (0 == conf->public_index_service) {
        //大致算出 node 的数量
        if (0 == conf->sqlite_mode)
            node_num = gnb_get_node_num_from_file(conf);
        else
            node_num = gnb_get_node_num_from_sqlite(conf);
    }
    ...
    size_t block_size = sizeof(uint32_t) * 256 + sizeof(gnb_ctl_magic_number_t) + sizeof(gnb_ctl_conf_zone_t) +
                        sizeof(gnb_ctl_core_zone_t) + sizeof(gnb_ctl_status_zone_t) + sizeof(gnb_ctl_node_zone_t) +
                        sizeof(gnb_node_t) * node_num + sizeof(gnb_block32_t) * 5;
...
static void setup_log_ctx(gnb_conf_t *conf, gnb_log_ctx_t *log) {
    ...
    if ('\0' != conf->log_path[0]) {
        ...
        if (0 == conf->public_index_service) {
            ...
            snprintf(log->log_file_name_debug, PATH_MAX + NAME_MAX, "%s/debug.%d.log", conf->log_path,
                     conf->udp4_ports[0]);
            snprintf(log->log_file_name_error, PATH_MAX + NAME_MAX, "%s/error.%d.log", conf->log_path,
                     conf->udp4_ports[0]);
        }
...
gnb_core_t *gnb_core_create(gnb_conf_t *conf) {
    ...
    if (0 == gnb_core->conf->lite_mode) {
        ...
        //加载 node.conf
        if(0 ==gnb_core->conf->sqlite_mode)
            local_node_file_config(gnb_core);
        else
            local_node_sqlite_config(gnb_core);
...
void primary_process_loop(gnb_core_t *gnb_core) {
    ...
    do {
        ...
        if (gnb_core->ctl_block->status_zone->keep_alive_ts_sec - last_exec_loop_script_ts_sec >
            GNB_EXEC_SCRIPT_INTERVAL_TIME_SEC) {
